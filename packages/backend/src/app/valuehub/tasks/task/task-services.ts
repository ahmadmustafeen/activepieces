...
export const taskService = {
    ...
    async onAIGeneratedGoal(history: any, createdBy: string) {
        try {
            const completion = await openai.createChatCompletion({  model: 'gpt-3.5-turbo', messages: [ { role: 'assistant', content: CreateGoalPrompt(history) }, { role: ChatCompletionRequestMessageRoleEnum.User, content: 'Create the goal' }] })
            ...
            const { title = '', category = [], description = [] } = parsedData

            const goalData = await goalService.create({ title, description: [description], category, createdBy })
